...
// Waveshare ESP32-S3-Touch-LCD-1.28 onboard GC9A01A display pins
#define TFT_BL   2
#define REED_PIN 15
#define TFT_DC   8
#define TFT_CS   9
...
const uint8_t BRIGHTNESS_OPEN = 255;
const uint8_t BRIGHTNESS_CLOSED = 40;
...
// Colors
#define COLOR_TACTICAL_BLUE 0x4AFF
#define COLOR_DIM_BLUE      0x0013
...
// Use hardware SPI, mapped to the Waveshare onboard LCD pins in setup().
...
const uint16_t objectsColor[] = {GC9A01A_GREEN, COLOR_DIM_BLUE, GC9A01A_RED, GC9A01A_ORANGE, GC9A01A_YELLOW};
...
void setup()
{
  ...
  // Map ESP32-S3 hardware SPI to the Waveshare LCD pins before tft.begin().
...
// Backlight / reed switch functions
void updateBacklight()
{
  bool doorOpen = digitalRead(REED_PIN) == LOW;   // NO reed closes to GND when magnet is near
  uint8_t targetBrightness = doorOpen ? BRIGHTNESS_OPEN : BRIGHTNESS_CLOSED;
...
void bootTacticalDisplay(void)
{
  ...
  {
    drawRectAtAngleCoord(undetectedObjects[i], COLOR_DIM_BLUE);
  }
  smartDelay(500);
...
void writeRadarOn(int letterSpeed, int wordSpeed, uint16_t color)
{
  ...
  {
    displayAurebeshString(WORDS[i], i, letterSpeed, color);
    smartDelay(wordSpeed);
  }
}
...
void blinkTacticalText(const char *s, int line, int durationMs, int speed)
{
  Timer time;
  time = initTimer(durationMs);
  int color = 0;
...
void displayAurebeshString(const char *s, int line, int speed, uint16_t color)
{
  tft.setCursor(TEXT_LINES[line][0], TEXT_LINES[line][1]);
